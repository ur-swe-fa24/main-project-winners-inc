...
std::vector<int> Map::getRoute(Room& startRoom, Room& endRoom) const {
    std::vector<int> route;
    std::queue<Room*> queue;
    std::unordered_map<Room*, Room*> cameFrom;  // To track the path
    ...
    while (!queue.empty()) {
        ...
        if (current == &endRoom) {
            ...
            while (current != nullptr) {
                route.push_back(current->getRoomId());
                current = cameFrom[current];
            }
            std::reverse(route.begin(), route.end());  // Reverse the path
            return route;
        }

        for (Room* neighbor : current->neighbors) {
            // Skip if neighbor is already visited or blocked by a virtual wall
            if (cameFrom.find(neighbor) == cameFrom.end() && !isVirtualWallBetween(current, neighbor)) {
                queue.push(neighbor);
    ...
    return route;  // Return an empty route if no path found
}
