...
void addVirtualWall(int wallID, Room *room1, Room *room2){VirtualWall *newVW = new VirtualWall(wallID, room1, room2)}
    ...
    while (!currentRoom.empty()) {
        ...
        for (Room *neighbor : current->neighbors) {
            if (std::find(proposedRoute.begin(), proposedRoute.end(), neighbor->roomId) == proposedRoute.end()) {
                // currentRoom.push_back(neighbor);                // Push neighbor to currentRoom so this room becomes
                // current
